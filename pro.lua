...
local creds = window:Tab("公告",'7733993211')
    ...
    bin:Label("主作者:鹤")
    bin:Label("此脚本为免费pro版，需要白名单")
    bin:Label("免费的免费的禁止圈狗")
    bin:Label("算是半缝合吧")
    bin:Label("感谢支持我的人，不支持我的也没必要诋毁")
...
local function HeartbeatUpdate()
	...
	FpsLabel.Text = ("鹤水印:"..os.date("%H").."时"..os.date("%M").."分"..os.date("%S")).."秒"
end
Start = tick()
...
CreditText.Text = "原作者: topit 汉化:鹤玄奘 "
...
local creds = credits:Tab("自瞄", "7733666258")

local credits = creds:section("请只选一个自瞄，否则会叠加", true)
...
local creds = window:Tab("范围",'7733770689')
...
local credits = creds:section("旋转", true)
credits:Button(
    "旋转10",
    function()
    local speed = 10
...
local credits = creds:section("floor2", true)    
credits:Button(
        "NBDOORS",
        function()
...
local credits = creds:section("酒店", true)
...
local credits = creds:section("门", true)   
...
credits:Toggle("自动挥舞", "ATHW", false, function(ATHW)
...
credits:Toggle("自动售卖", "ATSELL", false, function(ATSELL)
...
credits:Toggle("购买排名", "ATBP", false, function(ATBP)
...
credits:Toggle("购买腰带", "ATBYD", false, function(ATBYD)
...
credits:Toggle("购买技能", "ATB", false, function(ATB)
...
credits:Toggle("自动购买剑", "ATBS", false, function(ATBS)
...
local creds = window:Tab("『举重模拟器』",'7733674153')

local credits = credits:section("主要功能",true)

credits:Toggle("自动举重","text",false,function(s)
toggle = Value
...
credits:Toggle("自动卖","text",false,function(s)
tog = Value
   while tog do wait()
local ohTable1 = {
	[1] = "SellMuscle"
...
credits:Button("打开商店",function()
game:GetService("Players").LocalPlayer.PlayerGui["Main_Gui"]["UpgradeMenu_Frame"].Visible = true
end)

local creds = window:Tab("钓鱼模拟器",'7733674153')

local credits = UI3Tab4:section("主要功能",true)
...
local creds = window:Tab("自然灾害",'7733920226')

local credits = credits:section("功能",true)

credits:Toggle("地图投票界面", "Map Voting UI", false, function(bool)
...
credits:Toggle("开启游戏岛悬崖碰撞体积", "Togglelnfo", false, function(bool)
...
credits:Toggle("自动存活", "ToggleInfo", false, function(bool)
    _G.autowinfarm = bool;
    while wait(.1) do
...
local creds = window:Tab("力量传奇",'7733747106')

local credsxknnb = credits:section("传送功能",true)
credsxknnb:Button("回到出生点",function()
...
creds:Toggle("自动俯卧撑", "ATFWC", false, function(ATFWC)
    local player = game:GetService("Players").LocalPlayer
    local character = player.Character
    local originalPosition
...
creds:Toggle("自动收集物品", "AutoCollect", false, function(AutoCollect)
...
creds:Toggle("自动倒立身体", "ATDL", false, function(ATDL)
...
creds:Toggle("自动锻炼", "ATAAA", false, function(ATAAA)
...
creds:Toggle("自动重生", "ATRE", false, function(ATRE)
...
local creds = credits:section("跑步机",true)

creds:Toggle("沙滩跑步机10", "PPJ10", false, function(PPJ10)
...
creds:Toggle("健身房跑步机2000", "PPJ2000", false, function(PPJ2000)
...
creds:Toggle("神话健身房跑步机2000", "SHPPJ2000", false, function(SHPPJ2000)
...
creds:Toggle("永恒健身房跑步机3500", "YHPPJ3500", false, function(YHPPJ3500)
...
creds:Toggle("传奇健身房跑步机3000", "CQPPJ3000", false, function(CQPPJ3000)
...
local creds = credits:section("打石头",true)

creds:Toggle("石头0", "text",false,function(rock)
    getgenv().rock = rock
...
creds:Toggle("石头10", "text",false,function(rock)
...
creds:Toggle("石头100", "text",false,function(rock)
...
creds:Toggle("石头5000", "text",false,function(rock)
...
creds:Toggle("石头150000", "text",false,function(rock)
...
creds:Toggle("石头400000", "text",false,function(rock)
...
creds:Toggle("石头750000", "text",false,function(rock)
...
creds:Toggle("石头100万", "text",false,function(rock)
...
creds:Toggle("石头500万", "text",false,function(rock)
...
local creds = credits:section("深蹲架",true)

creds:Toggle("深蹲架沙滩", "text",false,function(rack)
...
creds:Toggle("深蹲架霜冻健身房", "text",false,function(rack)
...
creds:Toggle("深蹲架传奇健身房", "text",false,function(rack)
    getgenv().spam = rack
while getgenv().spam do
wait()
if game.Players.LocalPlayer.machineInUse.Value == nil then
...
creds:Toggle("深蹲架肌肉健身房", "text",false,function(rack)
    getgenv().spam = rack
while getgenv().spam do
wait()
if game.Players.LocalPlayer.machineInUse.Value == nil then
...
local creds = credits:section("引体向上",true)

creds:Toggle("引体向上海滩", "text",false,function(pull)
    if game.Players.LocalPlayer.leaderstats.Strength.Value >= 1000 then
getgenv().spam = pull
while getgenv().spam do
wait()
...
creds:Toggle("引体向上神话健身房", "text",false,function(pull)
    if game.Players.LocalPlayer.leaderstats.Strength.Value >= 4000 then
getgenv().spam = pull
while getgenv().spam do
wait()
...
local creds = credits:section("挥拳无间隔",true)

creds:Toggle("挥拳无间隔", "AL", false, function(Value)
    noCD = Value
    updatePunch()
...
local credits = window:Tab("一路向西",'7733770689')

local creds = credits:section("功能",true)

creds:Button("子弹追踪",function()
...
creds:Toggle("昵称显卡显示","text",false,function(Find_Required)
...
local credits = creds:section("玩家",true)
    credits:Button("玩家护盾电箱",function()
    local argsTemplate = {
    [1] = Vector3.new(),
    [2] = Vector3.new(0, 1, 0),
...
for _, tool in ipairs(backpack:GetChildren()) do
    ...
    credits:Button("枪械连射400发",function()
    local player = game.Players.LocalPlayer
local backpack = player.Backpack
...
local credits = creds:section("赠送",true)
    credits:Button("赠送弹窗循环",function()
    local Players = game:GetService("Players")
...
while true do
    ...
    credits:Button("一次性赠送弹窗",function()
    local Players = game:GetService("Players")
local ReplicatedStorage = game:GetService("ReplicatedStorage")
